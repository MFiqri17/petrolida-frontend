export interface IRegisterDataItem {
  id: number
  label: string
  name: string
  placeholder: string
  types: string
  pattern: RegExp | string
  path_twibbon?: string
}

interface FileInput {
  id: number
  label: string
  name: string
  placeholder: string
  types: 'file'
  path_twibbon: string
  pattern: RegExp
}

interface IRegisterData1 {
  General: IRegisterDataItem[]
  Leader: IRegisterDataItem[]
  'Member 1': IRegisterDataItem[]
  'Member 2': IRegisterDataItem[]
  'Member 3': IRegisterDataItem[]
  'Member 4': IRegisterDataItem[]
  Files: FileInput[]
}

interface IRegisterData2 {
  General: IRegisterDataItem[]
  Leader: IRegisterDataItem[]
  'Member 1': IRegisterDataItem[]
  'Member 2': IRegisterDataItem[]
  Files: FileInput[]
}



export const registerData: IRegisterData1 = {
  General: [
    {
      id: 1,
      label: 'Competition Type',
      name: 'event_id',
      placeholder: 'Oil Rig Design Competition',
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'University',
      name: 'university',
      placeholder: 'Enter your University',
      types: 'text',
      pattern: '',
    },
    {
      id: 3,
      label: 'Team Name',
      name: 'team_name',
      placeholder: "Enter your team's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Amount',
      name: 'amount',
      placeholder: 'Enter your amounts',
      types: 'text',
      pattern: '',
    },
  ],
  Leader: [
    {
      id: 1,
      label: 'Leader Name',
      name: 'members[0][name]',
      placeholder: "Enter your leader's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'Leader Major',
      name: 'members[0][major]',
      placeholder: "Enter your leader's major",
      types: 'text',
      pattern: '',
    },
    {
      id: 3,
      label: 'Leader Batch',
      name: 'members[0][batch]',
      placeholder: "Enter your leader's batch",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Leader ID (NIK/Passport)',
      name: 'members[0][identity_id]',
      placeholder: "Enter your leader's ID",
      types: 'text',
      pattern: '',
    },
    {
      id: 5,
      label: 'Leader Email',
      name: 'members[0][email]',
      placeholder: "Enter your leader's email",
      types: 'email',
      pattern: /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i,
    },
    {
      id: 6,
      label: 'Leader Phone Number',
      name: 'members[0][phone]',
      placeholder: "Enter your leader's phone number",
      types: 'text',
      pattern: '',
    },
    {
      id: 7,
      label: 'Upload KTP/Passport',
      name: 'members[0][identity_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 8,
      label: 'Upload Students ID Card',
      name: 'members[0][student_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 9,
      label: 'Upload your Twibbon',
      name: 'members[0][twibbon_path]',
      placeholder: '',
      types: 'file',
      path_twibbon: 'https://www.twibbonize.com/petrolida2023twibbon',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
  'Member 1': [
    {
      id: 1,
      label: 'Member 1 Name',
      name: 'members[1][name]',
      placeholder: "Eneter your member 1's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'Member 1 Major',
      name: 'members[1][major]',
      placeholder: "Enter your member 1's major",
      types: 'text',
      pattern: '',
    },
    {
      id: 3,
      label: 'Member 1 Batch',
      name: 'members[1][batch]',
      placeholder: "Enter your member 1's batch",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Member 1 ID (NIK/Passport)',
      name: 'members[1][identity_id]',
      placeholder: "Enter your member 1's ID",
      types: 'text',
      pattern: '',
    },
    {
      id: 5,
      label: 'Member 1 Email',
      name: 'members[1][email]',
      placeholder: "Enter your member 1's email",
      types: 'email',
      pattern: /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i,
    },
    {
      id: 6,
      label: 'Member 1 Phone Number',
      name: 'members[1][phone]',
      placeholder: "Enter your member 1's phone number",
      types: 'text',
      pattern: '',
    },
    {
      id: 7,
      label: 'Upload KTP/Passport',
      name: 'members[1][identity_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 8,
      label: 'Upload Students ID Card',
      name: 'members[1][student_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 9,
      label: 'Upload your Twibbon',
      name: 'members[1][twibbon_path]',
      placeholder: '',
      types: 'file',
      path_twibbon: 'https://www.twibbonize.com/petrolida2023twibbon',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
  'Member 2': [
    {
      id: 1,
      label: 'Member 2 Name',
      name: 'members[2][name]',
      placeholder: "Eneter your member 2's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'Member 2 Major',
      name: 'members[2][major]',
      placeholder: "Enter your member 2's major",
      types: 'text',
      pattern: '',
    },
    {
      id: 3,
      label: 'Member 2 Batch',
      name: 'members[2][batch]',
      placeholder: "Enter your member 2's batch",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Member 2 ID (NIK/Passport)',
      name: 'members[2][identity_id]',
      placeholder: "Enter your member 2's ID",
      types: 'text',
      pattern: '',
    },
    {
      id: 5,
      label: 'Member 2 Email',
      name: 'members[2][email]',
      placeholder: "Enter your member 2's email",
      types: 'email',
      pattern: /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i,
    },
    {
      id: 6,
      label: 'Member 2 Phone Number',
      name: 'members[2][phone]',
      placeholder: "Enter your member 2's phone number",
      types: 'text',
      pattern: '',
    },
    {
      id: 7,
      label: 'Upload KTP/Passport',
      name: 'members[2][identity_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 8,
      label: 'Upload Students ID Card',
      name: 'members[2][student_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 9,
      label: 'Upload your Twibbon',
      name: 'members[2][twibbon_path]',
      placeholder: '',
      types: 'file',
      path_twibbon: 'https://www.twibbonize.com/petrolida2023twibbon',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
  'Member 3': [
    {
      id: 1,
      label: 'Member 3 Name',
      name: 'members[3][name]',
      placeholder: "Eneter your member 3's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'Member 3 Major',
      name: 'members[3][major]',
      placeholder: "Enter your member 3's major",
      types: 'text',
      pattern: '',
    },
    {
      id: 3,
      label: 'Member 3 Batch',
      name: 'members[3][batch]',
      placeholder: "Enter your member 3's batch",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Member 3 ID (NIK/Passport)',
      name: 'members[3][identity_id]',
      placeholder: "Enter your member 3's ID",
      types: 'text',
      pattern: '',
    },
    {
      id: 5,
      label: 'Member 3 Email',
      name: 'members[3][email]',
      placeholder: "Enter your member 3's email",
      types: 'email',
      pattern: /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i,
    },
    {
      id: 6,
      label: 'Member 3 Phone Number',
      name: 'members[3][phone]',
      placeholder: "Enter your member 3's phone number",
      types: 'text',
      pattern: '',
    },
    {
      id: 7,
      label: 'Upload KTP/Passport',
      name: 'members[3][identity_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 8,
      label: 'Upload Students ID Card',
      name: 'members[3][student_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 9,
      label: 'Upload your Twibbon',
      name: 'members[3][twibbon_path]',
      placeholder: '',
      types: 'file',
      path_twibbon: 'https://www.twibbonize.com/petrolida2023twibbon',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
  'Member 4': [
    {
      id: 1,
      label: 'Member 4 Name',
      name: 'members[4][name]',
      placeholder: "Eneter your member 4's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'Member 4 Major',
      name: 'members[4][major]',
      placeholder: "Enter your member 4's major",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Member 4 Batch',
      name: 'members[4][batch]',
      placeholder: "Enter your member 4's batch",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Member 4 ID (NIK/Passport)',
      name: 'members[4][identity_id]',
      placeholder: "Enter your member 4's ID",
      types: 'text',
      pattern: '',
    },
    {
      id: 5,
      label: 'Member 4 Email',
      name: 'members[4][email]',
      placeholder: "Enter your member 4's email",
      types: 'email',
      pattern: /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i,
    },
    {
      id: 6,
      label: 'Member 4 Phone Number',
      name: 'members[4][phone]',
      placeholder: "Enter your member 4's phone number",
      types: 'text',
      pattern: '',
    },
    {
      id: 7,
      label: 'Upload KTP/Passport',
      name: 'members[4][identity_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 8,
      label: 'Upload Students ID Card',
      name: 'members[4][student_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 9,
      label: 'Upload your Twibbon',
      name: 'members[4][twibbon_path]',
      placeholder: '',
      types: 'file',
      path_twibbon: 'https://www.twibbonize.com/petrolida2023twibbon',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
  Files: [
    {
      id: 1,
      label: 'Upload Transfer Receipt',
      name: 'transfer_receipt',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
}

export const registerData2 : IRegisterData2 = {
  General: [
    {
      id: 1,
      label: 'Competition Type',
      name: 'event_id',
      placeholder: 'Oil Rig Design Competition',
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'University',
      name: 'university',
      placeholder: 'Enter your University',
      types: 'text',
      pattern: '',
    },
    {
      id: 3,
      label: 'Team Name',
      name: 'team_name',
      placeholder: "Enter your team's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Amount',
      name: 'amount',
      placeholder: 'Enter your amounts',
      types: 'text',
      pattern: '',
    },
  ],
  Leader: [
    {
      id: 1,
      label: 'Leader Name',
      name: 'members[0][name]',
      placeholder: "Enter your leader's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'Leader Major',
      name: 'members[0][major]',
      placeholder: "Enter your leader's major",
      types: 'text',
      pattern: '',
    },
    {
      id: 3,
      label: 'Leader Batch',
      name: 'members[0][batch]',
      placeholder: "Enter your leader's batch",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Leader ID (NIK/Passport)',
      name: 'members[0][identity_id]',
      placeholder: "Enter your leader's ID",
      types: 'text',
      pattern: '',
    },
    {
      id: 5,
      label: 'Leader Email',
      name: 'members[0][email]',
      placeholder: "Enter your leader's email",
      types: 'email',
      pattern: /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i,
    },
    {
      id: 6,
      label: 'Leader Phone Number',
      name: 'members[0][phone]',
      placeholder: "Enter your leader's phone number",
      types: 'text',
      pattern: '',
    },
    {
      id: 7,
      label: 'Upload KTP/Passport',
      name: 'members[0][identity_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 8,
      label: 'Upload Students ID Card',
      name: 'members[0][student_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 9,
      label: 'Upload your Twibbon',
      name: 'members[0][twibbon_path]',
      placeholder: '',
      types: 'file',
      path_twibbon: 'https://www.twibbonize.com/petrolida2023twibbon',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
  'Member 1': [
    {
      id: 1,
      label: 'Member 1 Name',
      name: 'members[1][name]',
      placeholder: "Eneter your member 1's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'Member 1 Major',
      name: 'members[1][major]',
      placeholder: "Enter your member 1's major",
      types: 'text',
      pattern: '',
    },
    {
      id: 3,
      label: 'Member 1 Batch',
      name: 'members[1][batch]',
      placeholder: "Enter your member 1's batch",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Member 1 ID (NIK/Passport)',
      name: 'members[1][identity_id]',
      placeholder: "Enter your member 1's ID",
      types: 'text',
      pattern: '',
    },
    {
      id: 5,
      label: 'Member 1 Email',
      name: 'members[1][email]',
      placeholder: "Enter your member 1's email",
      types: 'email',
      pattern: /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i,
    },
    {
      id: 6,
      label: 'Member 1 Phone Number',
      name: 'members[1][phone]',
      placeholder: "Enter your member 1's phone number",
      types: 'text',
      pattern: '',
    },
    {
      id: 7,
      label: 'Upload KTP/Passport',
      name: 'members[1][identity_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 8,
      label: 'Upload Students ID Card',
      name: 'members[1][student_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 9,
      label: 'Upload your Twibbon',
      name: 'members[1][twibbon_path]',
      placeholder: '',
      types: 'file',
      path_twibbon: 'https://www.twibbonize.com/petrolida2023twibbon',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
  'Member 2': [
    {
      id: 1,
      label: 'Member 2 Name',
      name: 'members[2][name]',
      placeholder: "Eneter your member 2's name",
      types: 'text',
      pattern: '',
    },
    {
      id: 2,
      label: 'Member 2 Major',
      name: 'members[2][major]',
      placeholder: "Enter your member 2's major",
      types: 'text',
      pattern: '',
    },
    {
      id: 3,
      label: 'Member 2 Batch',
      name: 'members[2][batch]',
      placeholder: "Enter your member 2's batch",
      types: 'text',
      pattern: '',
    },
    {
      id: 4,
      label: 'Member 2 ID (NIK/Passport)',
      name: 'members[2][identity_id]',
      placeholder: "Enter your member 2's ID",
      types: 'text',
      pattern: '',
    },
    {
      id: 5,
      label: 'Member 2 Email',
      name: 'members[2][email]',
      placeholder: "Enter your member 2's email",
      types: 'email',
      pattern: /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i,
    },
    {
      id: 6,
      label: 'Member 2 Phone Number',
      name: 'members[2][phone]',
      placeholder: "Enter your member 2's phone number",
      types: 'text',
      pattern: '',
    },
    {
      id: 7,
      label: 'Upload KTP/Passport',
      name: 'members[2][identity_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 8,
      label: 'Upload Students ID Card',
      name: 'members[2][student_card]',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
    {
      id: 9,
      label: 'Upload your Twibbon',
      name: 'members[2][twibbon_path]',
      placeholder: '',
      types: 'file',
      path_twibbon: 'https://www.twibbonize.com/petrolida2023twibbon',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
  Files: [
    {
      id: 1,
      label: 'Upload Transfer Receipt',
      name: 'transfer_receipt',
      placeholder: '',
      types: 'file',
      path_twibbon: '',
      pattern: /image\/(png|jpg|jpeg)/i,
    },
  ],
}
